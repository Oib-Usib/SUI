...
async function buildCJS(entryPoints: string[], { name }: PackageJSON) {
	await build({
		format: 'cjs',
		logLevel: 'error',
		target: 'es2020',
...
async function buildESM(entryPoints: string[], { name }: PackageJSON) {
	await build({
		format: 'esm',
		logLevel: 'error',
		target: 'es2020',
...
async function buildImportDirectories({ exports, name }: PackageJSON) {
	if (!exports) {
		return;
	}
